        ...
        if(false == stop){
            ...
            if (!cekHurufBlok[huruf][blok].equals("0")
                ...
                for (int iblok = 0; iblok < koordinatPerBlok[blok].length; iblok++) {
                    int kolomrandom = ToMathOptimization.multiplicativeCRNG(getAGeneration(),
                            10);
                    if (kolomrandom >= 10) {
                        kolomrandom = ToMathOptimization.multiplicativeCRNG(getAGeneration(),
                                10);
                    }
                    if (kolomrandom <= 0) {
                        ...
                        for (int i = 0; i < 20; i++) {
                            System.out.println("- "
                                    + ToMathOptimization.multiplicativeCRNG(getAGeneration(),
                                    10) + " - ");
                        }
                        System.out.println("REPEAT MCRNG = " + ToMathOptimization.getRepeat());
                        System.out.println("Z = " + ToMathOptimization.getZ());
                        ToMathOptimization.setZ(12357);
                        System.out.println("Z = " + ToMathOptimization.getZ());
                        kolomrandom = ToMathOptimization.multiplicativeCRNG(getAGeneration(),
                                10);
...
//			for (int i = 0; i < lenyap; i++) {
            int xy = ToMathOptimization.multiplicativeCRNG(aElimination[iaElimination],
                    65536);
            xy = xy % 81;
